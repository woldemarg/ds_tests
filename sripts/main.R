...
names(events_sum_up_df) <- c("date",
                             ...
                             change_pseudo_num_cols_to_factors <- function(data, lim = 15) {
                               ...
                               d <- d2 %>% select(target, encoded_company, month, day_of_week)
                               ...
                               for (i in 1:10) {
                                 ...
                                 mean_model <- trainData %>%
                                   group_by(encoded_company, month, day_of_week) %>%
                                   summarize(predicts = mean(target))
                                 
                                 testData <- testData %>%
                                   left_join(mean_model, by = c("encoded_company", "month", "day_of_week"))
